...
```{r}
sparsity_threshold_values <- seq(0, 1, by = 0.1)
sparsity_filtered_text.dt_list <- list()

for (threshold in sparsity_threshold_values) {
  sparsity_filtered_text.dt <- filtered_text.dt[abs(sparsity) >= threshold]
  sparsity_filtered_text.dt[, sparsity_threshold := threshold]
  sparsity_filtered_text.dt_list[[as.character(threshold)]] <- sparsity_filtered_text.dt
}

filtered_text.dt <- rbindlist(sparsity_filtered_text.dt_list)
```
...
```{r}
# Create the abs_corr_threshold values ranging from 0.0 to 1.0 
# with a step of 0.1
abs_corr_threshold_values <- seq(0, 1, by = 0.1)
...
filtered_data_list <- list()
for (threshold in sparsity_threshold_values) {
  filtered_data <- filtered_both.xy[abs(weight) >= threshold]
  filtered_data[, abs_corr_threshold := threshold]
  filtered_data_list[[as.character(threshold)]] <- filtered_data
}
filtered_both.xy <- rbindlist(filtered_data_list)
...
```{r}
sparsity_counts_dt <- filtered_text.dt[, .(count = .N), by = sparsity_threshold]
sparsity_counts_dt
```
...
viz <- animint(
  ...
  selectSparsity=ggplot()+
    ggtitle("Select Sparsity threshold") +
    geom_tallrect(aes(
      xmin=sparsity_threshold-0.1,
      xmax=sparsity_threshold+0.1),
      clickSelects="sparsity_threshold",
      #showSelected = "abs_corr_threshold",
      alpha=0.5,
      data=sparsity_counts_dt)+
    theme_bw()+
    theme(panel.margin=grid::unit(0, "lines"))+
    geom_line(aes(
      sparsity_threshold, count),
      data=sparsity_counts_dt),
  ...
  scatterSparsity=ggplot()+
    ...
    geom_point(aes(
      ...
      showSelected = c("pair","abs_corr_threshold", "sparsity_threshold"),
      color="black",
      size= 4,
    ...
    geom_point(aes(
      p_cor, sparsity, color = group),
      clickSelects="pair",
      showSelected = c("abs_corr_threshold","sparsity_threshold"),
      showSelected = c("abs_corr_threshold","sparsity_threshold"),
      data=filtered_text.dt),
  ...
  source = "https://github.com/tdhock/necromass/blob/main/figure-one-network.Rmd"
  )
...
```{r}
servr::httd("/private/var/folders/jf/1lv8wgr503396_w00202kgmw0000gp/T/RtmpvKnGAM/file29c622021e2")
```
