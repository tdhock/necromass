...
group_dict <- list(
  ...
  G2 = list(name = "Group 2", start = 1, end = 23),
  G3 = list(name = "Group 3", start = 24, end = 35),
  G4 = list(name = "Group 4", start = 36, end = 46),
  G5 = list(name = "Group 5", start = 47, end = 57),
  G6 = list(name = "Group 6", start = 58, end = 69)
)
options(warn = -1)
...
for (i in 1:ncol(group_comb)) {
  ...
  sub_dt[, r_squared_diff := diff(r_squared), by = pair]
...
viz <- animint(
  ...
  scatterSparsity=ggplot()+
    ...
    geom_point(aes(
      r_squared, sparsity, color = group),
      clickSelects="pair",
      data=text.dt),
  ...
  scatterSparsityDiff=ggplot()+
    ...
    geom_point(aes(
      r_squared_diff, sparsity_diff, color = group_pair),
      clickSelects="pair",
      data=group.dt),
...
```{r}
servr::httd("/private/var/folders/jf/1lv8wgr503396_w00202kgmw0000gp/T/RtmpKBFtYE/file7457281f4546")
```
